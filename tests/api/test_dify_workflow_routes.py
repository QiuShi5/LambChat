...
from src.plugins.dify_workflow import routes as workflow_routes
...
def _enabled_app(fake_service) -> FastAPI:
    ...
    app.include_router(workflow_routes.router, prefix="/api/plugins/dify-workflow")
    app.dependency_overrides[api_deps.get_current_user_required] = _workflow_user
    app.dependency_overrides[workflow_routes.get_dify_workflow_service] = lambda: fake_service
    return app
...
class _FakeCredential:
    def __init__(self) -> None:
        ...
        self.description = "Dify imported credential"
...
class _FakeService:
    ...
    async def import_dify_workflow(self, **kwargs):
...
def _assert_workflow_version_interface(payload: dict, *, version_id: str) -> None:
    assert payload["io_contract"]["plugin_id"] == "dify_workflow"
...
@pytest.mark.asyncio
async def test_dify_workflow_credential_routes_mask_secret_values() -> None:
    service = _FakeService()
    app = _enabled_app(service)
    transport = ASGITransport(app=app)

    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        listed = await client.get("/api/plugins/dify-workflow/credentials")
        upserted = await client.put(
            "/api/plugins/dify-workflow/credentials",
            json={
                "ref": "llm:provider_credential_id:openai-main",
                "type": "model",
                "label": "OpenAI main",
                "description": "Dify imported credential",
                "secret": "sk-should-not-return",
                "metadata": {"provider": "openai"},
            },
        )
        deleted = await client.delete("/api/plugins/dify-workflow/credentials/wfc-1")
...
@pytest.mark.asyncio
async def test_dify_workflow_routes_list_get_and_import() -> None:
    service = _FakeService()
    app = _enabled_app(service)
    transport = ASGITransport(app=app)

    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        listed = await client.get("/api/plugins/dify-workflow/workflows?skip=2&limit=10")
        fetched = await client.get("/api/plugins/dify-workflow/workflows/wf-1")
        versions = await client.get("/api/plugins/dify-workflow/workflows/wf-1/versions")
        input_schema = await client.get(
            "/api/plugins/dify-workflow/workflows/wf-1/input-schema?version_id=wfv-2"
        )
        io_contract = await client.get(
            "/api/plugins/dify-workflow/workflows/wf-1/io-contract?version_id=wfv-2"
        )
        ...
        versioned = await client.post(
            "/api/plugins/dify-workflow/workflows/wf-1/versions",
        ...
        published = await client.post(
            "/api/plugins/dify-workflow/workflows/wf-1/publish",
            json={"version_id": "wfv-2"},
        )
        validated = await client.post(
            "/api/plugins/dify-workflow/workflows/wf-1/validate",
            json={"version_id": "wfv-2"},
        )
        unpublished = await client.post(
            "/api/plugins/dify-workflow/workflows/wf-1/unpublish",
        )
        ran = await client.post(
            "/api/plugins/dify-workflow/workflows/wf-1/run",
            json={"input": {"name": "LambChat"}, "mode": "sync", "version_id": "wfv-2"},
        )
        runs = await client.get("/api/plugins/dify-workflow/workflows/wf-1/runs?limit=10")
        events = await client.get(
            "/api/plugins/dify-workflow/workflows/wf-1/runs/wfr-1/events"
        )
        event_stream = await client.get(
            "/api/plugins/dify-workflow/workflows/wf-1/runs/wfr-1/events/stream"
        )
        cancelled = await client.post(
            "/api/plugins/dify-workflow/workflows/wf-1/runs/wfr-1/cancel"
        )
        resumed = await client.post(
            "/api/plugins/dify-workflow/workflows/wf-1/runs/wfr-1/resume",
            json={"approved": True, "comment": "OK"},
        )
        node_types = await client.get("/api/plugins/dify-workflow/node-types")
        deleted = await client.delete("/api/plugins/dify-workflow/workflows/wf-1")
    ...
    assert node_payload["plugin_id"] == "dify_workflow"
...
@pytest.mark.asyncio
async def test_dify_workflow_route_forwards_editor_dsl_version_payload() -> None:
    service = _FakeService()
    app = _enabled_app(service)
    transport = ASGITransport(app=app)
    source_payload = {
...
@pytest.mark.asyncio
async def test_dify_workflow_route_saves_editor_dsl_through_real_service_and_storage() -> None:
...
@pytest.mark.asyncio
async def test_dify_workflow_route_runs_editor_saved_llm_workflow_through_real_runtime() -> None:
    from src.plugins.dify_workflow.service import DifyWorkflowService
    from src.plugins.dify_workflow.storage import DifyWorkflowStorage
...
@pytest.mark.asyncio
async def test_dify_workflow_route_runs_published_editor_version_by_default() -> None:
    from src.plugins.dify_workflow.service import DifyWorkflowService
    from src.plugins.dify_workflow.storage import DifyWorkflowStorage
...
@pytest.mark.asyncio
async def test_dify_workflow_run_routes_include_nested_output_contract_paths() -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        runs = await client.get("/api/plugins/dify-workflow/workflows/wf-1/runs")
        events = await client.get(
            "/api/plugins/dify-workflow/workflows/wf-1/runs/wfr-1/events"
        )
        event_stream = await client.get(
...
@pytest.mark.asyncio
async def test_dify_workflow_pending_approval_route_returns_user_scoped_paused_runs() -> None:
    service = _FakeService()
    app = _enabled_app(service)
    transport = ASGITransport(app=app)

    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get("/api/plugins/dify-workflow/approvals/pending?skip=1&limit=5")
    ...
    assert payload["runs"][0]["next_action"] == {
        ...
        "resume": {
            ...
            "method": "POST",
            "path": "/api/plugins/dify-workflow/workflows/wf-1/runs/wfr-1/resume",
...
@pytest.mark.asyncio
async def test_dify_workflow_route_guard_blocks_disabled_plugin() -> None:
    class _FailService:
        async def list_workflows(self, **kwargs):
    ...
    app.include_router(workflow_routes.router, prefix="/api/plugins/dify-workflow")
    app.dependency_overrides[api_deps.get_current_user_required] = _workflow_user
    app.dependency_overrides[workflow_routes.get_dify_workflow_service] = lambda: _FailService()

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get("/api/plugins/dify-workflow/workflows")

    assert response.status_code == 503
    assert response.json()["detail"]["error"] == "plugin_unavailable"
    assert response.json()["detail"]["plugin_id"] == DIFY_WORKFLOW_PLUGIN_ID


@pytest.mark.asyncio
async def test_create_app_mounts_dify_workflow_routes_behind_runtime_guard() -> None:
    app = api_main.create_app()
    app.dependency_overrides[api_deps.get_current_user_required] = _workflow_user
    app.dependency_overrides[workflow_routes.get_dify_workflow_service] = lambda: _FakeService()

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get(
            "/api/plugins/dify-workflow/workflows",
...
@pytest.mark.asyncio
async def test_create_app_dify_workflow_route_reaches_service_when_runtime_enabled() -> None:
    service = _FakeService()
    app = api_main.create_app()
    app.state.plugin_runtime.enable_plugin(DIFY_WORKFLOW_PLUGIN_ID)
    app.dependency_overrides[api_deps.get_current_user_required] = _workflow_user
    app.dependency_overrides[workflow_routes.get_dify_workflow_service] = lambda: service

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get(
            "/api/plugins/dify-workflow/workflows?skip=2&limit=10&query=invoice&status=published",
            headers={"Authorization": "Bearer test-token"},
        )

    assert response.status_code == 200
    payload = response.json()
    assert payload["plugin_id"] == DIFY_WORKFLOW_PLUGIN_ID
...
@pytest.mark.asyncio
async def test_dify_workflow_route_returns_structured_503_when_service_factory_fails(
    ...
    app.state.plugin_runtime.enable_plugin(DIFY_WORKFLOW_PLUGIN_ID)
    app.dependency_overrides[api_deps.get_current_user_required] = _workflow_user
    monkeypatch.setattr(workflow_routes, "create_dify_workflow_service", _create_service)
...
@pytest.mark.asyncio
async def test_dify_workflow_import_returns_400_for_invalid_source_content() -> None:
    from src.plugins.dify_workflow.service import DifyWorkflowService

    app = _enabled_app(DifyWorkflowService())
    transport = ASGITransport(app=app)
...
@pytest.mark.asyncio
async def test_dify_workflow_explicit_version_404_details_echo_requested_version() -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        schema_response = await client.get(
            "/api/plugins/dify-workflow/workflows/wf-1/input-schema?version_id=wfv-other"
        )
        publish_response = await client.post(
            "/api/plugins/dify-workflow/workflows/wf-1/publish",
...
@pytest.mark.asyncio
async def test_dify_workflow_non_versioned_404_details_do_not_require_request_version() -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        versions_response = await client.get("/api/plugins/dify-workflow/workflows/missing/versions")
        unpublish_response = await client.post(
            "/api/plugins/dify-workflow/workflows/missing/unpublish"
        )
...
async def test_dify_workflow_run_route_accepts_deferred_modes(mode: str) -> None:
    service = _FakeService()
    app = _enabled_app(service)
    transport = ASGITransport(app=app)
...
@pytest.mark.asyncio
async def test_dify_workflow_run_event_stream_waits_for_new_events() -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get(
            "/api/plugins/dify-workflow/workflows/wf-1/runs/wfr-1/events/stream?poll_ms=100&timeout_ms=2000"
        )
...
@pytest.mark.asyncio
async def test_dify_workflow_run_events_route_returns_503_when_storage_fails() -> None:
...
@pytest.mark.asyncio
async def test_dify_workflow_run_event_stream_emits_error_when_later_poll_loses_run() -> None:
...
@pytest.mark.asyncio
async def test_dify_workflow_run_event_stream_emits_error_when_later_poll_fails() -> None:
...
@pytest.mark.asyncio
async def test_dify_workflow_run_event_stream_returns_503_when_initial_poll_fails() -> None:
...
@pytest.mark.asyncio
async def test_dify_workflow_run_event_stream_stops_when_run_is_paused() -> None:
...
@pytest.mark.asyncio
async def test_dify_workflow_run_event_stream_marks_running_timeout_as_waiting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get(
            "/api/plugins/dify-workflow/workflows/wf-1/runs/wfr-1/events/stream?poll_ms=100&timeout_ms=1000"
        )
