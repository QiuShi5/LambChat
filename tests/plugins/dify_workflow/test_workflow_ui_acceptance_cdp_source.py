...
def test_cdp_ui_acceptance_saves_graph_before_publish_and_run() -> None:
    ...
    assert 'click_testid(client, "dify-workflow-save-graph", timeout=30)' in source
    assert 'saved_version_id = saved_evidence["latestVersionId"]' in source
    assert 'click_testid(client, "dify-workflow-publish-latest", timeout=30)' in source
    assert 'click_testid(client, "dify-workflow-run-version", timeout=30)' in source

    save_index = source.index('click_testid(client, "dify-workflow-save-graph", timeout=30)')
    publish_index = source.index('click_testid(client, "dify-workflow-publish-latest", timeout=30)')
    run_index = source.index('click_testid(client, "dify-workflow-run-version", timeout=30)', publish_index)
...
def test_cdp_ui_acceptance_creates_and_configures_human_approval_node_before_save() -> None:
    ...
    assert 'click_testid(client, "dify-workflow-node-add-human_approval", timeout=30)' in source
...
def test_cdp_ui_acceptance_verifies_node_event_focus_and_rename_reset() -> None:
    ...
    assert 'document.querySelector(\'[data-testid="dify-workflow-selected-node-id"]\')' in source
    ...
    run_index = source.index('click_testid(client, "dify-workflow-run-version", timeout=30)')
...
def test_cdp_ui_acceptance_runs_human_approval_pause_and_resume_flow() -> None:
    ...
    assert 'click_testid(client, "dify-workflow-approval-approve", timeout=30)' in source
...
def test_cdp_ui_acceptance_uses_stable_run_controls_and_canvas_status_selectors() -> None:
    ...
    assert 'set_select_by_testid(client, "dify-workflow-run-mode", mode)' in source
    assert "def canvas_run_status_state" in source
    assert "dify-workflow-canvas-run-status-start" in source
    assert "dify-workflow-canvas-run-status-answer" in source
    assert "dify-workflow-canvas-node-start" in source
    assert "dify-workflow-canvas-node-answer" in source
    ...
    run_index = source.index('click_testid(client, "dify-workflow-run-version", timeout=30)', choose_index)
...
def test_cdp_ui_acceptance_uses_stable_action_buttons_for_import_save_publish_and_approval() -> None:
    ...
    assert 'click_testid(client, "dify-workflow-import-submit", timeout=20)' in source
    assert 'click_testid(client, "dify-workflow-save-graph", timeout=30)' in source
    assert 'click_testid(client, "dify-workflow-publish-latest", timeout=30)' in source
    assert 'click_testid(client, "dify-workflow-approval-approve", timeout=30)' in source
    ...
    import_index = source.index('click_testid(client, "dify-workflow-import-submit", timeout=20)')
    save_index = source.index('click_testid(client, "dify-workflow-save-graph", timeout=30)', import_index)
    publish_index = source.index('click_testid(client, "dify-workflow-publish-latest", timeout=30)', save_index)
    run_index = source.index('click_testid(client, "dify-workflow-run-version", timeout=30)', publish_index)
...
def test_cdp_ui_acceptance_verifies_dedicated_editor_and_run_routes() -> None:
    ...
    assert "hasCanvas: Boolean(document.querySelector('[data-testid=\"dify-workflow-canvas\"]'))" in source
    assert "hasReactFlow: Boolean(document.querySelector('[data-testid=\"dify-workflow-react-flow\"]'))" in source
    assert "hasNodePalette: Boolean(document.querySelector('[data-testid=\"dify-workflow-node-palette\"]'))" in source
...
def test_browser_workflow_mock_uses_real_dify_workflow_frontend_contribution_shape() -> None:
    source = MOCK_SCRIPT.read_text(encoding="utf-8")

    assert 'id: "dify_workflow:workflows-tab"' in source
    assert 'panel: "dify_workflow:workflows-panel"' in source
    assert 'insert_after: "agent-team"' in source
    assert 'id: "dify_workflow:workflow-editor-tab"' in source
    assert 'path: "/workflows/:workflowId/editor"' in source
    assert 'panel: "dify_workflow:workflow-editor-panel"' in source
    assert 'id: "dify_workflow:workflow-run-tab"' in source
    assert 'path: "/workflows/:workflowId/runs/:runId"' in source
    assert 'panel: "dify_workflow:workflow-run-panel"' in source
    assert 'id: "dify_workflow:workflows-panel", tab: "workflows", renderer: "dify_workflow.WorkflowPanel"' in source
    assert 'id: "dify_workflow:workflow-editor-panel", tab: "workflows-editor", renderer: "dify_workflow.WorkflowPanel"' in source
    assert 'id: "dify_workflow:workflow-run-panel", tab: "workflows-run", renderer: "dify_workflow.WorkflowPanel"' in source
    assert 'id: "dify_workflow:workflows-nav"' in source
...
def test_browser_workflow_mock_can_script_drop_connect_and_save_editor_graph() -> None:
    ...
    assert "document.querySelector('[data-testid=\"dify-workflow-add-edge-target-' + sourceNodeId + '\"]')" in source
    ...
    assert 'nodeCards.includes("dify-workflow-node-card-node_5")' in source
...
def test_editor_interaction_cdp_replays_drop_connect_save_smoke() -> None:
    ...
    assert "dify-workflow-interface-contract" in source
    assert "dify-workflow-interface-entry" in source
    assert "dify-workflow-interface-exit" in source
    assert "dify-workflow-interface-schema" in source
    ...
    assert 'click_editor_testid(client, "dify-workflow-save-graph", timeout=30)' in source
    ...
    save_index = source.index('click_editor_testid(client, "dify-workflow-save-graph", timeout=30)')
...
def test_chat_replay_approval_cdp_resumes_replayed_paused_workflow_tool_result() -> None:
    ...
    assert "part.result.plugin_id === 'dify_workflow'" in source
...
def test_agent_team_approval_cdp_resumes_nested_paused_workflow_run() -> None:
    ...
    assert "part.plugin_id === 'dify_workflow'" in source
...
def test_scheduled_approval_cdp_exposes_resume_handoff_and_run_route() -> None:
    ...
    assert "bodyText.includes('workflow_resume')" in source
    assert "bodyText.includes('/api/plugins/dify-workflow/workflows/wf-browser/runs/run-approval-browser/resume')" in source
    assert "bodyText.includes('/api/plugins/dify-workflow/approvals/pending')" in source
...
def test_disabled_workflow_contributions_cdp_proves_runtime_filtering() -> None:
    ...
    assert "difyWorkflowPlugin(false)" in mock_source
