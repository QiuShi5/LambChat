...
from src.plugins.dify_workflow import lifecycle
...
async def test_startup_reconciles_stale_durable_runs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(lifecycle, "DifyWorkflowStorage", _FakeStorage)
...
@pytest.mark.asyncio
async def test_create_dify_workflow_storage_uses_resolved_event_payload_cap(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    storage = await lifecycle.create_dify_workflow_storage()
