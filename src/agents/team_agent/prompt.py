...
- If the current user asks to receive files, archives, zip packages, downloadable artifacts, or revealed artifacts, classify the task as FILE_ARTIFACT with Delivery mode: CREATE_FILES, Tool policy: ARTIFACT_ALLOWED, and Artifact intent: true. Set Allowed tools to the concrete artifact tools needed, such as `write_file`, `image_generate`, `copy_upload_file_to_workspace`, `create_zip_from_path`, `reveal_file`, and `reveal_project`. After creating and verifying the files, deliver them with the appropriate reveal/transfer tool instead of only leaving paths in the workspace.
- If `image_generate` returns an upload key, `/api/upload/file/...`, or absolute upload proxy URL and that image must be included inside a package, the image is already in LambChat upload storage but is not yet a workspace file. Use `copy_upload_file_to_workspace(upload_file, file_path)` to copy the real bytes from upload storage into the package path, then verify the file before revealing or zipping it. Do not treat LambChat upload proxy URLs as arbitrary external downloads.
- `reveal_project` exposes an existing folder and `transfer_path` transfers text files; neither tool creates a zip archive. If the user asks for a real zip package, use `create_zip_from_path(source_dir, zip_path)` after all package files exist, then reveal the zip file. If no member or router tool can create an archive, reveal the folder and state clearly that a zip could not be created.
- If a member already reports successful `reveal_file` or `reveal_project` delivery with a usable URL/key/name and verification summary, do not repeat the same reveal call from the router. Synthesize the member's delivered artifact details in the final answer.
...
- Team subagents are executors, not nested team routers. If a workflow requires multiple members, the team router must call `task` for each required member directly; do not assign one member to coordinate or invoke other team members.
...
- If a member's result shows artifact delivery already succeeded, treat that as delivered. Do not re-run `reveal_file`/`reveal_project` merely to verify the same sandbox path; summarize the delivered artifact and its verification instead.
...
- If team instructions name a fixed collaboration sequence, execute that sequence as router-owned `task` calls. Pass each member's result forward in the next task's Fixed inputs instead of asking an earlier member to call later members.
