...
from src.plugins.dify_workflow.user_context import workflow_user_for_user_id
...
async def workflow_list(
    ...
    user = await _workflow_user_from_runtime(runtime)
    if user is None:
        return _json({"plugin_id": "dify_workflow", "scope": scope, "workflows": []})
    try:
        service = await create_dify_workflow_service()
    ...
    return _json({"plugin_id": "dify_workflow", "scope": scope, "workflows": workflows})
