...
function getTypeIcon(type: string) {
  switch (type) {
    case "user":
      return <User size={10} className="opacity-60" />;
    case "feedback":
      return <MessageSquare size={10} className="opacity-60" />;
    case "project":
      return <FolderGit2 size={10} className="opacity-60" />;
    case "reference":
      return <BookOpen size={10} className="opacity-60" />;
    default:
      return <Tag size={10} className="opacity-60" />;
  }
}
...
function ScoreBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5 w-full">
      ...
      <span className="text-[9px] tabular-nums text-theme-text-tertiary w-6 text-right">
        {pct}
      </span>
    </div>
  );
    ...
    <div className="p-4 sm:p-5 space-y-4">
      ...
      {resultQuery && (
        <div className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-[color-mix(in_srgb,var(--theme-primary)_7%,var(--theme-bg-card))] border border-[color-mix(in_srgb,var(--theme-primary)_16%,var(--theme-border))] shadow-[0_10px_24px_-22px_color-mix(in_srgb,var(--theme-primary)_45%,transparent)]">
          <Search size={13} className="text-[var(--theme-primary)] shrink-0" />
          <span className="text-xs sm:text-sm text-theme-text truncate flex-1">
            {resultQuery}
          </span>
          <ToolHoverCopyButton
            text={resultQuery}
          ...
          />
          {searchMode && (
            <span className="shrink-0 text-[9px] px-1.5 py-0.5 rounded-md bg-theme-bg-card ring-1 ring-[color-mix(in_srgb,var(--theme-primary)_14%,var(--theme-border))] text-theme-text-secondary font-medium">
              {searchModeLabel}
            </span>
          )}
        </div>
              ...
              return (
                <div
                  ...
                  className={clsx(
                    ...
                    isTopResult
                      ? "border-[color-mix(in_srgb,var(--theme-primary)_26%,var(--theme-border))]"
                      : "border-theme-border",
                  )}
                >
                  {/* Top accent for top result */}
                  {isTopResult && (
                    <div className="h-[2px] bg-gradient-to-r from-[var(--theme-primary)] via-[color-mix(in_srgb,var(--theme-primary)_46%,transparent)] to-transparent" />
                  )}
                  ...
                  <div className="px-3.5 py-3 space-y-2.5">
                    ...
                    <div className="flex items-start gap-2">
                      <div className="flex-1 min-w-0">
                        {mem.title && (
                          <div className="text-xs sm:text-sm font-semibold text-theme-text truncate leading-snug tracking-tight">
                            {mem.title}
                          </div>
                        )}
                      </div>
                    ...
                    {(mem.summary || mem.preview) && (
                      <p className="text-[11px] sm:text-xs text-theme-text-secondary/80 leading-relaxed line-clamp-3">
                        {mem.summary || mem.preview}
                      </p>
                    )}
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        className={clsx(
                          "inline-flex items-center gap-1 px-2 py-[3px] rounded-md text-[10px] font-medium",
                          typeStyle,
                        )}
                      >
                        <span
                      ...
                      {/* Date */}
                      <span className="inline-flex items-center gap-1 text-[10px] text-theme-text-tertiary ml-auto">
                    ...
                    {mem.staleness_warning && (
                      <div className="flex items-start gap-1.5 px-2.5 py-2 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50 dark:border-amber-800/30">
                        <AlertTriangle
                          size={11}
          ...
          <ToolInlineDetails>
            ...
            {resultQuery && (
              <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-[color-mix(in_srgb,var(--theme-primary)_6%,var(--theme-bg-card))] border border-[color-mix(in_srgb,var(--theme-primary)_12%,var(--theme-border))]">
                ...
                />
                <span className="text-[11px] text-theme-text-secondary truncate flex-1">
            ...
            {memories.length > 0 && (
              <div className="space-y-1">
                {memories.slice(0, 5).map((mem, i) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={clsx(
                          "inline-flex items-center px-1.5 py-[2px] rounded text-[9px] font-medium",
                          typeStyle,
                        )}
                      >
                        {t(`memory.type.${mem.type}`, mem.type)}
                      </span>
                      <span className="text-[10px] text-theme-text-secondary truncate flex-1">
                        {mem.title ||
                          (mem.summary
                            ? mem.summary.slice(0, 40)
                            : (mem.preview || "").slice(0, 40))}
                      </span>
                      <span className="text-[9px] tabular-nums text-theme-text-tertiary shrink-0">
                ...
                {memories.length > 5 && (
                  <div className="text-[10px] text-theme-text-tertiary px-2.5">
