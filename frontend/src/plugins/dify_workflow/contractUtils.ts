  ...
  for (const [field, rawSchema] of Object.entries(properties)) {
    ...
    if (expectedTypes.length > 0 && !workflowInputMatchesType(fieldValue, expectedTypes)) {
      return {
        ...
        messageKey: "difyWorkflow.validation.inputTypeMismatch",
    ...
    if (
      ...
      return {
        ...
        messageKey: "difyWorkflow.validation.inputOptionMismatch",
...
export function workflowInputDraftStatus(
  ...
  try {
    ...
    return {
      ...
      messageKey: "difyWorkflow.validation.invalidJson",
    };
  }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    return {
      message: "Input must be a JSON object",
      messageKey: "difyWorkflow.validation.inputObjectRequired",
    };
  }
  ...
  if (missing.length > 0) {
    return {
      ...
      messageKey: "difyWorkflow.validation.missingRequiredInput",
